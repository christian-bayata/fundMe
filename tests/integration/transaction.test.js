...
const { withdrawFromAccount } = require("../../api/v1/controllers/transaction");
...
describe("Transaction Controller", () => {
  ...
  describe("Withdraw from user account", () => {
    it("should fail if user's token is not valid", async () => {
      const token = new User({ _id: mongoose.Types.ObjectId(), email: "user1@gmail.com", isAdmin: false }).generateJsonWebToken();
      ...
      const response = await request(server).post(`${baseURL}/withdraw-user-account`).set("authorization", "vhjhiu80uigfytuytugvjoiugejvbmgjyiuhewk").send(payload);
      expect(response.status).toBe(401);
      expect(response.body.message).toMatch(/unauthenticated/i);
    });
    ...
    it("should fail if user does not provide amount", async () => {
      ...
      const response = await request(server).post(`${baseURL}/withdraw-user-account`).set("authorization", token).send(payload);
      expect(response.status).toBe(400);
      expect(response.body.message).toMatch(/provide the amount/i);
    });
    ...
    it("should succeed if all reqirements are met", async () => {
      const token = new User({ _id: mongoose.Types.ObjectId(), email: "user1@gmail.com", isAdmin: false }).generateJsonWebToken();
      const decode = jwt.verify(token, process.env.JWT_SECRET_KEY, (err, decoded) => decoded);
      ...
      expect(response.status).toBe(200);
      expect(response.body.message).toMatch(/successful withdrawal/i);
    });
  });
});
